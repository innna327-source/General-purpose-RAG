...
def normalize_for_match(text: str) -> str:
    ...
    t = _collapse_cjk_spaces(t)          # 处理 PDF 字符间空格
...
def _collapse_cjk_spaces(t: str) -> str:
    """合并 PDF 字符级空格：'大 模 型' → '大模型'（不影响中英混排的词间空格）"""
    return _CJK_SPACE_RE.sub(
...
def _clean_basic(text: str, char_pattern: re.Pattern = _DEFAULT_CHAR_PATTERN) -> str:
    ...
    t = t.replace("\x00", " ")
    t = _collapse_cjk_spaces(t)        # 修复 PDF 字符间空格（必须在空格归一化之前）
    t = re.sub(r"[^\S\r\n]+", " ", t)  # 多余空格（保留换行）
    t = re.sub(r"\n{3,}", "\n\n", t)  # 连续换行
